...
/**
DMAP TAG: Query converted Needs Manual Intervention : Identifier236
DMAP ConvertedQuery - SELECT coalesce(MAX(CAST(T1.SUB_UR_NO AS numeric)), 0) AS MAX_SUB_UR_NO FROM IP_UR_F5 T1 WHERE T1.UR_NO='urNo.trim()'
**/
	...
	@Override
	public IpUrF5 getID(IpUrF5Id id) { // DMAP Comment : Dead Code Detected - The Following Method has no reference getID
		Session session = getSessionFactory().getCurrentSession();
		Criteria criteria = session.createCriteria(IpUrF5.class);
	...
	@Override
	public List<IpvRetriveF5> retriveF5(String vip, String port, String urNo) { // DMAP Comment : Dead Code Detected - The Following Method has no reference retriveF5
		Session session = getSessionFactory().getCurrentSession();
...
/**
DMAP TAG: Query converted but found same: Identifier235
DMAP ConvertedQuery - UPDATE IP_UR_F5 set IS_IMPACT='N' WHERE UR_NO=?
**/
...
/**
DMAP TAG: Query converted but found same: Identifier234
DMAP ConvertedQuery - DELETE FROM IP_UR_F5 F5 WHERE F5.UR_NO=?
**/
	...
	public void delete(String urNo, String subUrNo) { // DMAP Comment : Dead Code Detected - The Following Method has no reference delete
		Session session = getSessionFactory().getCurrentSession();
String sql = "DELETE FROM IP_UR_F5 FW WHERE FW.UR_NO=? AND FW.SUB_UR_NO IN (" + subUrNo + ")";//String sql = "DELETE FROM IP_UR_F5 FW WHERE FW.UR_NO=? AND FW.SUB_UR_NO IN ("+subUrNo+")";
/**
DMAP TAG: Query converted: Identifier233
DMAP ConvertedQuery - DELETE FROM IP_UR_F5 FW WHERE FW.UR_NO=? AND FW.SUB_UR_NO IN (subUrNo)
**/
	...
	@Override
	public void updateSubUrStatus(IpUrF5Id id, String status,IpUser user) { // DMAP Comment : Dead Code Detected - The Following Method has no reference updateSubUrStatus
		Session session = getSessionFactory().getCurrentSession();
...
/**
DMAP TAG: Query converted Needs Manual Intervention : Identifier232
DMAP ConvertedQuery - update IP_UR_F5 set SUB_UR_STATUS=? , UPDATE_BY=? , UPDATE_DATE=statement_timestamp() WHERE (SUB_UR_NO=? and UR_NO=?)
**/
...
/**
DMAP TAG: Query converted but found same: Identifier231
DMAP ConvertedQuery - DELETE from IP_UR_F5 WHERE (UR_NO=?) AND (CHANGE_TYPE=?)
**/
...
/**
DMAP TAG: Query converted Needs Manual Intervention : Identifier230
DMAP ConvertedQuery - UPDATE IP_UR_F5 SET CHANGE_TYPE = NULL WHERE (UR_NO=?) AND (CHANGE_TYPE=?)
**/
	...
	@Override
	public List<IpUrF5> waitDeleteUr(String urNo) { // DMAP Comment : Dead Code Detected - The Following Method has no reference waitDeleteUr
		Session session = getSessionFactory().getCurrentSession();
		Criteria criteria = session.createCriteria(IpUrF5.class);
		criteria.add(Restrictions.eq("id.urNo", urNo));
	...
	@Override
	public IpUrF5 fineIpUrF5(String urNo,String subUrNo) { // DMAP Comment : Dead Code Detected - The Following Method has no reference fineIpUrF5
		Session session = getSessionFactory().getCurrentSession();
		Criteria criteria = session.createCriteria(IpUrF5.class);
		criteria.add(Restrictions.eq("id.urNo", urNo));
